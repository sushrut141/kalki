...
ABSL_FLAG(std::string, statusz_listen_address, "0.0.0.0:8081", "statusz HTTP listen address");
...
ABSL_FLAG(double, similarity_threshold, 0.2, "Similarity threshold for embedding filtering");
...
DatabaseConfig LoadConfigFromFlags() {
  ...
  cfg.statusz_listen_address = absl::GetFlag(FLAGS_statusz_listen_address);
